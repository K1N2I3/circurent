...
2. **创建新的 API Key**
   ...
   - 格式类似：`sk-ant-api03-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`
...
3. **查看定价**
   - Claude Sonnet 4 的定价：
     - 输入：$3 / 1M tokens
     - 输出：$15 / 1M tokens
   - 每次图片分析大约消耗 1K-3K tokens
   - 成本约为 $0.002-0.01 每次分析
...
2. **添加 Anthropic API Key**
   ...
   ```
   - 将 `sk-ant-api03-your-actual-api-key-here` 替换为你的实际 API Key
...
- **每次图片分析**：
  - 输入 tokens：约 1K-2K（图片编码 + 提示词）
  - 输出 tokens：约 200-500（生成的描述）
  - **成本**：约 $0.002-0.01 每次
